...
import {
  probeUserApiConfigModelLlmProtocol,
  testUserApiProvider,
} from '@renderer/clients/api-config-client'
...
export async function probeModelLlmProtocolViaApi(params: {
  ...
  const response = await probeUserApiConfigModelLlmProtocol({
...
export function useProviderCardState({
  ...
  const handleSaveKey = useCallback(async () => {
    ...
    try {
      ...
      const res = await testUserApiProvider(payload)
