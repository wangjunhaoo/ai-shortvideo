...
interface VideoTimelinePanelProps {
  ...
  labels: VideoTimelinePanelLabels
}

export interface VideoTimelinePanelLabels {
  title: string
  linesCountLabel: (count: number) => string
  audioGeneratedCountLabel: (count: number) => string
}
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div className="text-left">
            <h3 className="font-bold text-[var(--glass-text-primary)]">{labels.title}</h3>
            <p className="text-sm text-[var(--glass-text-tertiary)]">
              {labels.linesCountLabel(allVoiceLines.length)}
              {labels.audioGeneratedCountLabel(allVoiceLines.filter((line) => line.audioUrl).length)}
            </p>
          </div>
        </div>
