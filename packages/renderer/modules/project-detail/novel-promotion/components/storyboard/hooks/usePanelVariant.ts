...
import { useCreateProjectPanelVariant, useRefreshEpisodeData } from '@renderer/modules/project-detail/novel-promotion/hooks/useRendererNovelPromotionOperations'
...
interface UsePanelVariantProps {
    ...
    messages: PanelVariantMessages
}

export interface PanelVariantMessages {
    generating: string
}

export function usePanelVariant({ projectId, episodeId, setLocalStoryboards, messages }: UsePanelVariantProps) {
    ...
    const generatePanelVariant = useCallback(async (
        ...
        setLocalStoryboards(prev => prev.map(sb => {
            ...
            const tempPanel: NovelPromotionPanel = {
                ...
                description: variant.description || messages.generating,
    ...
    }, [createPanelVariantMutation, messages.generating, onRefresh, setLocalStoryboards])
