...
import {
  ...
  useSelectProjectPanelCandidate,
...
interface UsePanelCandidatesProps {
  ...
  messages: PanelCandidateMessages
}

export interface PanelCandidateMessages {
  unknownError: string
  selectCandidateFailed: (error: string) => string
}
...
export function usePanelCandidates({
  ...
  const confirmPanelCandidate = useCallback(async (panelId: string, imageUrl: string) => {
    try {
      ...
      alert(messages.selectCandidateFailed(getErrorMessage(error, messages.unknownError)))
    }
  }, [
    candidateSystem,
    messages,
