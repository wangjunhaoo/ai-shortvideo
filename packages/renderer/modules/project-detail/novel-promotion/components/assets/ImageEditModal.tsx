...
export interface ImageEditModalLabels {
    title: string
    subtitle: string
    designInstructionRequired: string
...
export default function ImageEditModal({
    ...
    const handleSubmit = () => {
        if (!modifyPrompt.trim()) {
            alert(labels.designInstructionRequired)
            return
        }
        ...
        <div className="fixed inset-0 bg-[var(--glass-overlay)] z-50 flex items-center justify-center p-4">
            ...
            >
                <div className="p-6 border-b">
                    <h3 className="text-lg font-bold text-[var(--glass-text-primary)]">{labels.title}</h3>
                    <p className="text-sm text-[var(--glass-text-tertiary)] mt-1">{labels.subtitle}</p>
                </div>
                <div className="p-6 space-y-4">
                    <div>
                        <label className="block text-sm font-medium text-[var(--glass-text-secondary)] mb-2">{labels.editInstruction}</label>
                        <textarea
                            value={modifyPrompt}
                    ...
                    <div>
                        <label className="block text-sm font-medium text-[var(--glass-text-secondary)] mb-2">
                            {labels.referenceImages} <span className="text-[var(--glass-text-tertiary)] font-normal">{labels.referenceImagesHint}</span>
                        </label>
                        <input
                            ref={fileInputRef}
                            type="file"
