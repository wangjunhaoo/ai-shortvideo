...
import { useUploadProjectCharacterVoice } from '@renderer/modules/project-detail/novel-promotion/hooks/useRendererNovelPromotionOperations'
import { AppIcon } from '@/components/ui/icons'

export interface VoiceSettingsLabels {
    title: string
    noVoice: string
...
export default function VoiceSettings({
    ...
    const confirmUploadVoice = () => {
        return window.confirm(labels.uploadConfirm)
    }
    ...
    const handlePreviewVoice = async () => {
        ...
        try {
            ...
            if (shouldShowError(error)) {
                alert(labels.previewFailed(getErrorMessage(error, labels.unknownError)))
            }
            setIsPreviewingVoice(false)
        }
    }
    ...
    const handleUploadVoice = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        uploadVoice.mutate(
            { file, characterId },
            {
                ...
                onError: (error) => {
                    if (shouldShowError(error)) {
                        alert(labels.uploadFailed(error.message))
                    }
                },
                onSettled: () => {
        ...
        <div className={containerClass}>
            <div className={`${headerClass} ${hasCustomVoice ? 'border-[var(--glass-stroke-base)]' : 'border-[var(--glass-stroke-warning)]'}`}>
                ...
                <span className={`text-${compact ? 'xs' : 'sm'} font-medium ${hasCustomVoice ? 'text-[var(--glass-text-secondary)]' : 'text-[var(--glass-tone-warning-fg)]'}`}>
                    {labels.title}{!hasCustomVoice && <span className="text-[var(--glass-tone-warning-fg)]">({labels.noVoice})</span>}
                </span>
            </div>
            ...
            <div className="flex flex-wrap gap-2 w-full justify-center">
                ...
                >
                    <div className="flex items-center justify-center gap-1">
                        ...
                        <span>{uploadVoice.isPending ? labels.uploading : hasCustomVoice ? labels.uploaded : labels.uploadAudio}</span>
                    </div>
                </button>
                ...
                {onSelectFromHub && (
                    ...
                    >
                        <div className="flex items-center justify-center gap-1">
                            ...
                            <span>{labels.assetLibraryButton}</span>
                        </div>
                    </button>
                )}
            ...
            {hasCustomVoice && (
                ...
                >
                    <div className="flex items-center justify-center gap-2">
                        ...
                        {isPreviewingVoice ? labels.pause : labels.preview}
                    </div>
                </button>
            )}
        </div>
