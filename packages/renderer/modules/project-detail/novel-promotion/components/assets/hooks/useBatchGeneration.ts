...
import { useGenerateProjectCharacterImage, useGenerateProjectLocationImage } from '@renderer/modules/project-detail/novel-promotion/hooks/useRendererNovelPromotionOperations'
import {
...
interface UseBatchGenerationProps {
    ...
    messages: {
        generateAllNoop: string
        regenerateAllConfirm: string
        noAssetsToGenerate: string
    }
}
...
export function useBatchGeneration({
    ...
    const handleGenerateAllImages = async () => {
        ...
        if (tasks.length === 0) {
            alert(messages.generateAllNoop)
            return
        }
    ...
    const handleRegenerateAllImages = async () => {
        if (!confirm(messages.regenerateAllConfirm)) return
        ...
        if (tasks.length === 0) {
            alert(messages.noAssetsToGenerate)
            return
        }
