...
interface UseTTSGenerationProps {
    ...
    messages: {
        unknownError: string
        updateVoiceFailed: string
        voiceDesignSaved: (name: string) => string
        saveVoiceDesignFailed: (error: string) => string
    }
}
...
export function useTTSGeneration({
    ...
    const handleVoiceChange = async (characterId: string, voiceType: string, voiceId: string, customVoiceUrl?: string) => {
        try {
            ...
            _ulogError(messages.updateVoiceFailed, getErrorMessage(error, messages.unknownError))
        }
    }
    ...
    const handleVoiceDesignSave = async (voiceId: string, audioBase64: string) => {
        ...
        try {
            ...
            refreshAssets()
            alert(messages.voiceDesignSaved(voiceDesignCharacter.name))
        } catch (error: unknown) {
            alert(messages.saveVoiceDesignFailed(getErrorMessage(error, messages.unknownError)))
        } finally {
