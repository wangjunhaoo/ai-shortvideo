...
import { useCopyProjectAssetFromGlobal } from '@renderer/modules/project-detail/novel-promotion/hooks/useRendererNovelPromotionOperations'
...
interface UseAssetsCopyFromHubParams {
  ...
  showToast: ShowToast
  messages: {
    copySuccessCharacter: string
    copySuccessLocation: string
    copySuccessVoice: string
...
export function useAssetsCopyFromHub({ projectId, onRefresh, showToast, messages }: UseAssetsCopyFromHubParams) {
  ...
  const handleConfirmCopyFromGlobal = useCallback(async (globalAssetId: string) => {
    ...
    try {
      ...
      const successMsg = copyFromGlobalTarget.type === 'character'
        ? messages.copySuccessCharacter
        : copyFromGlobalTarget.type === 'location'
          ? messages.copySuccessLocation
          : messages.copySuccessVoice
      showToast(successMsg, 'success')
      setCopyFromGlobalTarget(null)
      await Promise.resolve(onRefresh())
    } catch (error: unknown) {
      if (!isAbortError(error)) {
        showToast(messages.copyFailed(getErrorMessage(error)), 'error')
      }
    } finally {
      setIsGlobalCopyInFlight(false)
    }
  }, [copyFromGlobalAsset, copyFromGlobalTarget, messages, onRefresh, showToast])
