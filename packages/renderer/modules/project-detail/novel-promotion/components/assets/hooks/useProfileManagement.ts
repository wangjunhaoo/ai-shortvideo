...
interface UseProfileManagementProps {
    ...
    messages: {
        parseFailed: string
        confirmSuccessGenerating: string
        confirmFailed: (error: string) => string
        noPendingCharacters: string
        batchConfirmPrompt: (count: number) => string
        batchConfirmSuccess: (count: number) => string
        batchConfirmFailed: (error: string) => string
...
export function useProfileManagement({
    ...
    const handleEditProfile = useCallback((characterId: string, characterName: string) => {
        ...
        if (!profileData) {
            showToast?.(messages.parseFailed, 'error')
            return
        }

        setEditingProfile({ characterId, characterName, profileData })
    }, [characters, messages.parseFailed, showToast])
    ...
    const handleConfirmProfile = useCallback(async (
        ...
        try {
            ...
            showToast?.(messages.confirmSuccessGenerating, 'success')
            refreshAssets()
        } catch (error: unknown) {
            const message = error instanceof Error ? error.message : messages.unknownError
            showToast?.(messages.confirmFailed(message), 'error')
        } finally {
            // 🔥 从确认中集合移除
    ...
    }, [confirmCharacterProfileMutation, messages, refreshAssets, showToast])

    // 批量确认所有角色
    const handleBatchConfirm = useCallback(async () => {
        if (unconfirmedCharacters.length === 0) {
            showToast?.(messages.noPendingCharacters, 'warning')
            return
        }

        if (!confirm(messages.batchConfirmPrompt(unconfirmedCharacters.length))) {
            return
        }
        ...
        try {
            ...
            showToast?.(messages.batchConfirmSuccess(confirmedCount), 'success')
            refreshAssets()
        } catch (error: unknown) {
            const message = error instanceof Error ? error.message : messages.unknownError
            showToast?.(messages.batchConfirmFailed(message), 'error')
        } finally {
            setBatchConfirmingLocal(false)
        }
    }, [batchConfirmProfilesMutation, messages, refreshAssets, showToast, unconfirmedCharacters.length])
    ...
    const handleDeleteProfile = useCallback(async (characterId: string) => {
        ...
        try {
            ...
            const message = error instanceof Error ? error.message : messages.unknownError
            showToast?.(messages.deleteFailed(message), 'error')
        } finally {
            setDeletingCharacterId(null)
        }
    }, [deleteCharacterMutation, messages, refreshAssets, showToast])
