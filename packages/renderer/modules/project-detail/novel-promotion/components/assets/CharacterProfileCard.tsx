...
export interface CharacterProfileCardLabels {
    roleLevelLabel: (roleLevel: string) => string
    deleteTitle: string
    genderLabel: string
    ageLabel: string
    eraLabel: string
    ...
    confirmAndGenerateLabel: string
}
...
interface CharacterProfileCardProps {
    ...
    isDeleting?: boolean
    labels: CharacterProfileCardLabels
}
...
export default function CharacterProfileCard({
    ...
    const roleLevelLabel = roleLevel
        ? labels.roleLevelLabel(roleLevel)
        ...
        <div className="bg-[var(--glass-bg-surface)] rounded-xl border border-[var(--glass-stroke-base)] p-4 hover:shadow-md transition-shadow">
            ...
            <div className="space-y-1.5 mb-3">
                <div className="flex items-center gap-2 text-sm">
                    <span className="text-[var(--glass-text-tertiary)] w-16">{labels.genderLabel}</span>
                    <span className="text-[var(--glass-text-primary)]">{profileData.gender}</span>
                </div>
                <div className="flex items-center gap-2 text-sm">
                    <span className="text-[var(--glass-text-tertiary)] w-16">{labels.ageLabel}</span>
                    <span className="text-[var(--glass-text-primary)]">{profileData.age_range}</span>
                </div>
                <div className="flex items-center gap-2 text-sm">
                    <span className="text-[var(--glass-text-tertiary)] w-16">{labels.eraLabel}</span>
                    <span className="text-[var(--glass-text-primary)]">{profileData.era_period}</span>
                </div>
                <div className="flex items-center gap-2 text-sm">
                    <span className="text-[var(--glass-text-tertiary)] w-16">{labels.classLabel}</span>
                    <span className="text-[var(--glass-text-primary)]">{profileData.social_class}</span>
                </div>
                {profileData.occupation && (
                    <div className="flex items-center gap-2 text-sm">
                        <span className="text-[var(--glass-text-tertiary)] w-16">{labels.occupationLabel}</span>
                        <span className="text-[var(--glass-text-primary)]">{profileData.occupation}</span>
                    </div>
                )}
                <div className="flex items-center gap-2 text-sm">
                    <span className="text-[var(--glass-text-tertiary)] w-16">{labels.personalityLabel}</span>
                ...
                <div className="flex items-center gap-2 text-sm">
                    <span className="text-[var(--glass-text-tertiary)] w-16">{labels.costumeLabel}</span>
                ...
                {profileData.primary_identifier && (
                    <div className="flex items-center gap-2 text-sm">
                        <span className="text-[var(--glass-text-tertiary)] w-16">{labels.identifierLabel}</span>
            ...
            <div className="flex gap-2">
                ...
                >
                    {isConfirming ? (
                        ...
                        labels.confirmAndGenerateLabel
                    )}
                </button>
            </div>
        </div>
