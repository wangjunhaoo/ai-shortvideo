...
import EmotionSettingsPanel, { type EmotionSettingsPanelLabels } from './EmotionSettingsPanel'
...
interface VoiceLineCardProps {
    ...
    labels: VoiceLineCardLabels
    emotionLabels: EmotionSettingsPanelLabels
}

export interface VoiceLineCardLabels {
    locateVideoTitle: string
    playTitle: string
    pauseTitle: string
    ...
    locateVideoLabel: string
    editLineTitle: string
    deleteLineTitle: string
    emotionConfiguredLabel: string
    emotionSettingsLabel: string
    voiceConfiguredLabel: string
    needVoiceLabel: string
}
...
export default function VoiceLineCard({
    ...
    const locateTitle = labels.locateVideoTitle
    ...
    return (
        ...
        >
                ...
                {line.audioUrl ? (
                    <div className="flex items-center justify-center gap-3">
                        ...
                        <button
                            ...
                            title={isPlaying ? labels.pauseTitle : labels.playTitle}
                        >
                            {isPlaying ? (
                    ...
                    <div className="mt-2 flex justify-end gap-0.5">
                        {hasPanelBinding && (
                            <button
                                ...
                                title={locateTitle}
                            >
                                <span>{labels.locateVideoLabel}</span>
                            </button>
                        )}
                        <button
                            onClick={() => onEdit(line)}
                    ...
                    <>
                        ...
                        >
                            ...
                            {line.emotionPrompt || (line.emotionStrength !== null && line.emotionStrength !== 0.4)
                                ? labels.emotionConfiguredLabel
                                : labels.emotionSettingsLabel}
                        </button>
            ...
            <div className="px-4 py-2.5 bg-[var(--glass-bg-muted)]/50 border-t border-[var(--glass-stroke-base)]/60 flex items-center justify-between gap-2">
                ...
                {hasVoice ? (
                    <span className="text-xs text-[var(--glass-tone-success-fg)] font-medium">{labels.voiceConfiguredLabel}</span>
                ) : (
                    <span className="text-xs text-[var(--glass-tone-warning-fg)] font-medium">{labels.needVoiceLabel}</span>
                )}
            </div>
        </div >
    )
