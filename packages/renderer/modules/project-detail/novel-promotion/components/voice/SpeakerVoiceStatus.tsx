...
interface SpeakerVoiceStatusProps {
    ...
    embedded?: boolean
    labels: SpeakerVoiceStatusLabels
}

export interface SpeakerVoiceStatusLabels {
    embeddedTitle: string
    embeddedSpeakersCountLabel: (count: number) => string
    linesCountLabel: (count: number) => string
        ...
        return (
            <div className="glass-surface px-4 py-3 mb-3 mx-4">
                <div className="flex items-center justify-between mb-2">
                    <h4 className="text-sm font-semibold text-[var(--glass-text-primary)]">{labels.embeddedTitle}</h4>
                    <span className="text-xs text-[var(--glass-text-tertiary)]">{labels.embeddedSpeakersCountLabel(speakers.length)}</span>
                </div>
                        ...
                        return (
                            ...
                            >
                                <div className="min-w-0">
                                    ...
                                    <div className="text-xs text-[var(--glass-text-tertiary)]">{labels.linesCountLabel(count)}</div>
                                </div>
                                <span className={`text-xs px-2 py-1 rounded-full ${hasVoice
                                    ...
                                    {hasVoice ? labels.configuredStatusLabel : labels.pendingStatusLabel}
                                </span>
                                {/* 无匹配角色时显示内联标记 */}
                    ...
                    return (
                        <div key={speaker} className="w-full sm:w-[280px] max-w-full flex items-center gap-1.5 px-3 py-2 rounded-xl border border-[var(--glass-stroke-base)] bg-[var(--glass-bg-surface-strong)]">
                            <div className="min-w-0">
                                ...
                                <div className="text-xs text-[var(--glass-text-tertiary)]">{labels.linesCountLabel(speakerStats[speaker])}</div>
                            </div>
