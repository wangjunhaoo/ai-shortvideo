...
interface SpeakerVoiceBindingDialogProps {
    ...
    labels: SpeakerVoiceBindingDialogLabels
}

export interface SpeakerVoiceBindingDialogLabels {
    title: (speaker: string) => string
    description: string
    selectFromLibraryLabel: string
    uploadAudioLabel: string
    aiDesignLabel: string
    selectFromLibraryDesc: string
    uploadAudioDesc: string
    aiDesignDesc: string
    uploadQwenHint: string
    voicePicker: VoicePickerDialogLabels
}
...
export default function SpeakerVoiceBindingDialog({
    ...
    const confirmUploadVoice = useCallback(() => {
        return window.confirm(labels.uploadQwenHint)
    }, [labels.uploadQwenHint])
    ...
    const handleVoiceSelected = useCallback((voice: {
        ...
        if (voice.voiceId) {
            ...
            alert(labels.uploadQwenHint)
        }
        setSubDialogOpen(false)
        onClose()
    }, [speaker, onBound, onClose, labels.uploadQwenHint])
        ...
        <>
            ...
            >
                ...
                <div className="px-5 py-3">
                    <SegmentedControl
                        options={[
                            { value: 'select' as const, label: labels.selectFromLibraryLabel },
                            { value: 'upload' as const, label: labels.uploadAudioLabel },
                            { value: 'design' as const, label: labels.aiDesignLabel },
                        ]}
                        value={activeTab}
                ...
                <div className="p-5">
                    <div className="text-center py-6">
                        ...
                        <p className="text-sm text-[var(--glass-text-secondary)] mb-4">
                            {activeTab === 'select' && labels.selectFromLibraryDesc}
                            {activeTab === 'upload' && labels.uploadAudioDesc}
                            {activeTab === 'design' && labels.aiDesignDesc}
                        </p>
                        <button
                            onClick={() => {
                        ...
                        >
                            {activeTab === 'select' && labels.selectFromLibraryLabel}
                            {activeTab === 'upload' && labels.uploadAudioLabel}
                            {activeTab === 'design' && labels.aiDesignLabel}
                        </button>
                    </div>
                </div>
            </div>
