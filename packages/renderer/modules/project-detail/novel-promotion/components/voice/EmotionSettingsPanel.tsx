...
interface EmotionSettingsPanelProps {
    ...
    labels: EmotionSettingsPanelLabels
}

export interface EmotionSettingsPanelLabels {
    emotionPromptLabel: string
    emotionPromptTip: string
    emotionPlaceholder: string
    emotionStrengthLabel: string
    flatLabel: string
    intenseLabel: string
    generateVoiceLabel: string
}
        ...
        <div className="px-4 py-3 bg-[var(--glass-tone-info-bg)] space-y-3">
            ...
            <div>
                <label className="block text-xs text-[var(--glass-tone-info-fg)] mb-1.5 font-medium">
                    {labels.emotionPromptLabel} <span className="text-[var(--glass-text-tertiary)] font-normal">{labels.emotionPromptTip}</span>
                </label>
                <input
                    type="text"
                    value={prompt}
                    onChange={(e) => handlePromptChange(e.target.value)}
                    placeholder={labels.emotionPlaceholder}
            ...
            <div>
                <label className="block text-xs text-[var(--glass-tone-info-fg)] mb-1.5 font-medium">
                    {labels.emotionStrengthLabel}: <span className="font-bold">{strength.toFixed(1)}</span>
                </label>
                <input
                    type="range"
                    min="0"
                ...
                <div className="flex justify-between text-[10px] text-[var(--glass-text-tertiary)] mt-1">
                    <span>{labels.flatLabel}</span>
                    <span>{labels.intenseLabel}</span>
                </div>
            </div>
