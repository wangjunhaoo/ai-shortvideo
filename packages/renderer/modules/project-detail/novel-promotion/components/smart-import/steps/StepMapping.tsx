...
import type { StepMappingLabels } from '../types'
        ...
        <div className="fixed inset-0 glass-overlay flex items-center justify-center z-50" onClick={onCloseDeleteConfirm}>
          <div className="glass-surface-modal p-6 w-full max-w-sm" onClick={e => e.stopPropagation()}>
            <div className="text-center mb-6">
              ...
              <h3 className="text-lg font-bold text-[var(--glass-text-primary)] mb-2">{labels.deleteConfirmTitle}</h3>
              <p className="text-[var(--glass-text-secondary)]">{labels.deleteConfirmMessage(deleteConfirm.title)}</p>
            </div>
        ...
        <div className="lg:col-span-1">
          <div className="bg-[var(--glass-bg-surface)] rounded-2xl border border-[var(--glass-stroke-base)] p-6 sticky top-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-semibold text-lg">{labels.episodeListLabel}</h3>
              <span className="text-sm text-[var(--glass-text-tertiary)]">{episodes.length} {labels.episodeListLabel}</span>
            </div>
              ...
              {episodes.map((ep, idx) => (
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    <input
                      type="text"
                      value={labels.episodeLabel(ep.number)}
                      onChange={(e) => {
                    ...
                    <div className="flex items-center gap-2">
                      <span className={`text-xs px-2 py-0.5 rounded-full ${selectedEpisode === idx ? 'bg-[var(--glass-accent-from)] text-white' : 'bg-[var(--glass-bg-muted)] text-[var(--glass-text-secondary)]'
                        ...
                        {ep.wordCount.toLocaleString()} {labels.wordsLabel}
                      </span>
                      {episodes.length > 1 && (
                        <button
                          onClick={(e) => {
                            e.stopPropagation()
                            onOpenDeleteConfirm(idx, labels.episodeLabel(ep.number))
                          }}
                          ...
                          title={labels.deleteEpisodeTitle}
                        >
                  ...
                  <input
                    ...
                    onClick={(e) => e.stopPropagation()}
                    placeholder={labels.episodePlaceholder}
            ...
            >
              ...
              {labels.addEpisodeLabel}
            </button>

            <div className="mt-4 pt-4 border-t border-[var(--glass-stroke-base)] space-y-2">
              <div className="flex justify-between text-sm">
                <span className="text-[var(--glass-text-secondary)]">{labels.averageWordsLabel}</span>
                <span className="font-semibold">
                  {episodes.length > 0 ? Math.round(episodes.reduce((sum, ep) => sum + ep.wordCount, 0) / episodes.length).toLocaleString() : 0} {labels.wordsLabel}
                </span>
              </div>
            </div>
          </div>
            ...
            <div className="bg-[var(--glass-bg-surface)] rounded-2xl border border-[var(--glass-stroke-base)] p-6">
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-4">
                  ...
                  />
                  <span className="text-sm text-[var(--glass-text-tertiary)]">{labels.episodeLabel(episodes[selectedEpisode].number)}</span>
                </div>
                <span className="text-sm text-[var(--glass-text-tertiary)]">{episodes[selectedEpisode].wordCount.toLocaleString()} {labels.wordsLabel}</span>
              </div>

              <div>
                <div className="flex items-center justify-between mb-3">
                  <label className="text-sm font-semibold text-[var(--glass-text-secondary)]">{labels.episodeContentLabel}</label>
                  <span className="text-sm text-[var(--glass-text-tertiary)]">{episodes[selectedEpisode].wordCount.toLocaleString()} {labels.wordsLabel}</span>
                </div>
                <textarea
                  rows={16}
              ...
              <div className="mt-4 p-4 bg-[var(--glass-tone-info-bg)] border border-[var(--glass-stroke-focus)] rounded-xl">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <p className="font-medium text-[var(--glass-text-primary)] mb-1">{labels.plotSummaryLabel}</p>
                    <p className="text-sm text-[var(--glass-text-primary)]">
                      {episodes[selectedEpisode].summary || labels.summaryPlaceholder}
                    </p>
                  </div>
                </div>
              </div>
