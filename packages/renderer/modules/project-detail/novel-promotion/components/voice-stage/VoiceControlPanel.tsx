...
import VoiceToolbar, { type VoiceToolbarLabels } from '../voice/VoiceToolbar'
import EmbeddedVoiceToolbar, { type EmbeddedVoiceToolbarLabels } from '../voice/EmbeddedVoiceToolbar'
import SpeakerVoiceStatus, { type SpeakerVoiceStatusLabels } from '../voice/SpeakerVoiceStatus'
import { AppIcon } from '@/components/ui/icons'

export interface VoiceControlPanelLabels {
  closeTitle: string
  toolbar: VoiceToolbarLabels
  embeddedToolbar: EmbeddedVoiceToolbarLabels
  speakerStatus: SpeakerVoiceStatusLabels
  lineEditor: {
    editTitle: string
    addTitle: string
    contentLabel: string
    ...
    noSpeakerOptions: string
    bindPanelLabel: string
    unboundPanel: string
    cancelLabel: string
...
interface VoiceControlPanelProps {
  ...
  labels: VoiceControlPanelLabels
}
          ...
          <div className="w-full max-w-xl bg-[var(--glass-bg-surface)] rounded-2xl shadow-2xl border border-[var(--glass-stroke-base)] p-5" onClick={(event) => event.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-[var(--glass-text-primary)]">
                {editingLineId ? labels.lineEditor.editTitle : labels.lineEditor.addTitle}
              </h3>
              <button
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-[var(--glass-text-secondary)] mb-1.5">{labels.lineEditor.contentLabel}</label>
                <textarea
                  value={editingContent}
                  onChange={(event) => onEditingContentChange(event.target.value)}
                  placeholder={labels.lineEditor.contentPlaceholder}
                  rows={4}
              ...
              <div>
                <label className="block text-sm font-medium text-[var(--glass-text-secondary)] mb-1.5">{labels.lineEditor.speakerLabel}</label>
                <select
                ...
                >
                  <option value="" disabled>{labels.lineEditor.selectSpeaker}</option>
                ...
                {speakerOptions.length === 0 && (
                  <p className="mt-1 text-xs text-[var(--glass-tone-warning-fg)]">{labels.lineEditor.noSpeakerOptions}</p>
                )}
              </div>

              <div>
                <label className="block text-sm font-medium text-[var(--glass-text-secondary)] mb-1.5">{labels.lineEditor.bindPanelLabel}</label>
                <select
                  value={editingMatchedPanelId}
                  onChange={(event) => onEditingMatchedPanelIdChange(event.target.value)}
                  className="w-full rounded-xl border border-[var(--glass-stroke-strong)] px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[var(--glass-tone-info-fg)]"
                >
                  <option value="">{labels.lineEditor.unboundPanel}</option>
            ...
            <div className="flex items-center justify-end gap-2 mt-6">
              ...
              >
                ...
                <span>{editingLineId ? labels.lineEditor.saveEditLabel : labels.lineEditor.saveAddLabel}</span>
              </button>
            </div>
          </div>
        </div>
