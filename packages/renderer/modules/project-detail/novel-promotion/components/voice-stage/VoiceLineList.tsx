...
import VoiceLineCard, { type VoiceLineCardLabels } from '../voice/VoiceLineCard'
import EmptyVoiceState, { type EmptyVoiceStateLabels } from '../voice/EmptyVoiceState'
import type { EmotionSettingsPanelLabels } from '../voice/EmotionSettingsPanel'
...
export interface VoiceLineListLabels {
  emptyState: EmptyVoiceStateLabels
  lineCard: VoiceLineCardLabels
  emotionSettings: EmotionSettingsPanelLabels
}
...
export default function VoiceLineList({
  ...
  if (voiceLines.length === 0) {
    return <EmptyVoiceState onAnalyze={onAnalyze} analyzing={analyzing} labels={labels.emptyState} />
  }
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-4 px-2 pt-4">
      {voiceLines.map((line) => (
        <VoiceLineCard
          ...
          labels={labels.lineCard}
          emotionLabels={labels.emotionSettings}
        />
      ))}
    </div>
  )
