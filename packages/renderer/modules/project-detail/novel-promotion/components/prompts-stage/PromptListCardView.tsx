...
export default function PromptListCardView({ runtime }: PromptListCardViewProps) {
  ...
  const cardLabels = labels.cardView
          ...
          <div key={shot.id} className="card-base overflow-hidden">
            <div className="aspect-video bg-[var(--glass-bg-muted)] flex items-center justify-center relative">
              {shot.imageUrl ? (
                <MediaImageWithLoading
                  src={shot.imageUrl}
                  alt={cardLabels.shotAlt(shot.shotId)}
              ...
              {shot.imageUrl && (
                <button
                  ...
                  title={cardLabels.regenerateImageTitle}
                >
            ...
            <div className="p-5 space-y-4">
                ...
                <div className="space-y-2 border-b pb-4">
                  ...
                  <div className="text-sm">
                    <div className="flex items-center justify-between mb-2">
                      <span className="font-semibold text-[var(--glass-text-primary)] text-base">{cardLabels.imagePromptLabel}</span>
                      {!isEditing && (
                        <button
                          ...
                          title={cardLabels.imagePromptLabel}
                        >
                    ...
                    {isEditing ? (
                      <div className="space-y-3">
                        <div>
                          <label className="block text-xs font-medium text-[var(--glass-text-secondary)] mb-1">{cardLabels.currentPromptLabel}</label>
                          <textarea
                            value={editValue}
                        ...
                        <div className="border-t pt-3">
                          <label className="block text-xs font-medium text-[var(--glass-text-secondary)] mb-1">
                            {cardLabels.aiInstructionLabel} <span className="text-[var(--glass-text-tertiary)]">{cardLabels.supportReferenceLabel}</span>
                          </label>
                          <div className="relative">
                            <textarea
                              ...
                              placeholder={cardLabels.instructionPlaceholder}
                              className="w-full px-3 py-2 border border-[var(--glass-stroke-strong)] rounded-lg focus:ring-2 focus:ring-[var(--glass-tone-info-fg)] focus:border-[var(--glass-stroke-focus)] text-sm resize-none"
                              rows={2}
                            />
                            ...
                            {showAssetPicker && (
                              <div className="absolute z-10 mt-1 w-full bg-[var(--glass-bg-surface)] border border-[var(--glass-stroke-strong)] rounded-lg shadow-lg max-h-48 overflow-y-auto">
                                <div className="p-2">
                                  <div className="text-xs font-medium text-[var(--glass-text-tertiary)] mb-2">{cardLabels.selectAssetLabel}</div>

                                  {assetLibraryCharacters.length > 0 && (
                                    <div className="mb-2">
                                      <div className="text-xs text-[var(--glass-text-tertiary)] mb-1">{cardLabels.characterLabel}</div>
                                  ...
                                  {assetLibraryLocations.length > 0 && (
                                    <div>
                                      <div className="text-xs text-[var(--glass-text-tertiary)] mb-1">{cardLabels.locationLabel}</div>
                          ...
                          {selectedAssets.length > 0 && (
                            <div className="flex flex-wrap gap-2 mt-3 p-2.5 bg-[var(--glass-bg-muted)]/50 rounded-lg border border-[var(--glass-stroke-base)]">
                              <div className="text-xs text-[var(--glass-text-tertiary)] font-medium w-full mb-1">{cardLabels.referencedAssetsLabel}</div>
                              {selectedAssets.map((asset, index) => (
                                ...
                                >
                                  ...
                                  <button
                                    ...
                                    title={cardLabels.removeAssetTitle}
                                  >
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
                ...
                {shot.scale && (
                  <div className="flex items-center gap-2">
                    <span className="text-[var(--glass-text-tertiary)] font-medium">{cardLabels.shotTypeLabel}</span>
                    <span className="text-[var(--glass-text-primary)]">{shot.scale}</span>
                  </div>
                )}
                {shot.locations && (
                  <div className="flex items-center gap-2">
                    <span className="text-[var(--glass-text-tertiary)] font-medium">{cardLabels.locationFieldLabel}</span>
                    <span className="text-[var(--glass-text-primary)]">{shot.locations}</span>
                  </div>
                )}
                {shot.module && (
                  <div className="flex items-center gap-2">
                    <span className="text-[var(--glass-text-tertiary)] font-medium">{cardLabels.modeLabel}</span>
