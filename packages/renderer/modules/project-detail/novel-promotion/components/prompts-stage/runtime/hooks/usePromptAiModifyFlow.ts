...
interface UsePromptAiModifyFlowParams {
  ...
  messages: Pick<PromptEditorMessages, 'enterInstruction' | 'modifyFailed' | 'unknownError'>
}
...
export function usePromptAiModifyFlow({
  ...
  const handleAiModify = async () => {
    ...
    if (!currentState || !currentState.aiModifyInstruction.trim()) {
      alert(messages.enterInstruction)
      return
    }
    ...
    try {
      ...
      if (shouldShowError(error)) {
        alert(messages.modifyFailed(getErrorMessage(error, messages.unknownError)))
      }
    } finally {
