...
interface CharacterPickerModalProps {
  ...
  onClose: () => void
  labels: CharacterPickerModalLabels
}

export interface CharacterPickerModalLabels {
  title: string
...
export function CharacterPickerModal({
  ...
  return (
    <GlassModalShell open onClose={onClose} size="md" title={labels.title}>
      <div className="max-h-[60vh] space-y-4 overflow-y-auto">
        {characters.length === 0 ? (
          <p className="py-8 text-center text-[var(--glass-text-secondary)]">{labels.noAssetsLabel}</p>
        ) : (
          characters.map(char => {
              ...
              <GlassSurface key={char.id} variant="panel" className="space-y-2 p-3">
                ...
                <div className="flex flex-wrap gap-2">
                  {appearances.map((app: CharacterAppearance) => {
                    const appearanceName = app.changeReason || labels.defaultAppearanceLabel
...
interface LocationPickerModalProps {
  ...
  onClose: () => void
  labels: LocationPickerModalLabels
}

export interface LocationPickerModalLabels {
  title: string
  noAssetsLabel: string
            ...
            {locations.map(loc => {
              ...
              return (
                ...
                >
                  ...
                  {isSelected ? (
                    <span className="text-xs text-[var(--glass-tone-success-fg)]">{labels.selectedLabel}</span>
                  ) : null}
                </button>
              )
            })}
