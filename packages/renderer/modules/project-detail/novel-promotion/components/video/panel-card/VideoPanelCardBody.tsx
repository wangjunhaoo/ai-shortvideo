  ...
  return (
    <div className="p-4 space-y-2">
      <div className="flex items-center justify-between text-xs">
        <span className="px-2 py-0.5 bg-[var(--glass-tone-info-bg)] text-[var(--glass-tone-info-fg)] rounded font-medium">{panel.textPanel?.shot_type || labels.unknownShotTypeLabel}</span>
        {panel.textPanel?.duration && <span className="text-[var(--glass-text-tertiary)]">{panel.textPanel.duration}{labels.durationSuffix}</span>}
      </div>
          ...
          <div className="mb-2 flex flex-wrap gap-1.5">
            {showsIncomingLinkBadge && (
              ...
              >
                ...
                {labels.asLastFrameForLabel(panelIndex)}
              </span>
            )}
            {showsOutgoingLinkBadge && (
              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-medium bg-[var(--glass-tone-info-bg)] text-[var(--glass-tone-info-fg)]">
                ...
                {labels.asFirstFrameForLabel(panelIndex + 2)}
              </span>
            )}
          </div>
        )}
        ...
        {showsPromptEditor && (
          <>
            <div className="flex items-center justify-between mb-1">
              <span className="text-xs font-medium text-[var(--glass-text-tertiary)]">{labels.promptLabel}</span>
            ...
            {promptEditor.isEditing ? (
              <div className="relative mb-3">
                ...
                <div className="absolute right-1 top-1 flex flex-col gap-1">
                  <button onClick={promptEditor.handleSave} disabled={promptEditor.isSavingPrompt} className="px-2 py-1 text-[10px] bg-[var(--glass-accent-from)] text-white rounded">{promptEditor.isSavingPrompt ? '...' : labels.saveLabel}</button>
                  <button onClick={promptEditor.handleCancelEdit} disabled={promptEditor.isSavingPrompt} className="px-2 py-1 text-[10px] bg-[var(--glass-bg-muted)] text-[var(--glass-text-secondary)] rounded">{labels.cancelLabel}</button>
                </div>
              </div>
            ) : (
              <div onClick={promptEditor.handleStartEdit} className="text-xs p-2 border border-[var(--glass-stroke-base)] rounded-lg bg-[var(--glass-bg-muted)] text-[var(--glass-text-secondary)] cursor-pointer">
                {promptEditor.localPrompt || <span className="text-[var(--glass-text-tertiary)] italic">{labels.clickToEditPromptLabel}</span>}
              </div>
            )}
            ...
            {showsFirstLastFrameActions ? (() => {
              ...
              return (
                <div className="mt-2 flex items-center gap-2">
                  ...
                  >
                    {isFirstLastFrameGenerated ? labels.firstLastFrameGeneratedLabel : taskStatus.isVideoTaskRunning ? taskStatus.taskRunningVideoLabel : labels.firstLastFrameGenerateLabel}
                  </button>
                  <div className="flex-1 min-w-0">
                    <ModelCapabilityDropdown
                      ...
                      capabilityFields={layout.flCapabilityFields.map((field) => ({
                        field: field.field,
                        label: labels.formatCapabilityLabel(field),
              ...
              <>
                <div className="flex items-center gap-2">
                  ...
                  >
                    {panel.videoUrl ? labels.hasSyncedLabel : taskStatus.isVideoTaskRunning ? taskStatus.taskRunningVideoLabel : labels.generateVideoLabel}
                  </button>
                  <div className="flex-1 min-w-0">
                    <ModelCapabilityDropdown
                      compact
                            ...
                            <div key={voiceLine.id} className="flex items-start gap-1.5 p-1.5 bg-[var(--glass-bg-muted)] rounded text-[10px]">
                              {voiceLine.audioUrl ? (
                                <button
                                  ...
                                  title={voiceManager.playingVoiceLineId === voiceLine.id ? labels.stopVoiceTitle : labels.playTitle}
                                >
                                ...
                                <button
                                  ...
                                  title={labels.generateAudioTitle}
                                >
                                  {isVoiceTaskRunning ? (
