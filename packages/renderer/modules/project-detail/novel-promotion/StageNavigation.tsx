...
import { useStageNavigationLabels } from './useStageNavigationLabels'
...
export function StageNavigation({
  ...
  const labels = useStageNavigationLabels()
  ...
  const stages = [
    { id: 'config', label: labels.configLabel, enabled: true },
    { id: 'assets', label: labels.assetsLabel, enabled: hasAudio || hasAssets },
    { id: 'storyboard', label: labels.storyboardLabel, enabled: hasTextStoryboards || hasStoryboards },
    { id: 'videos', label: labels.videosLabel, enabled: hasStoryboards || hasVideos },
    // 配音阶段只要有文本输入就可以启用，不受其他条件限制
    { id: 'voice', label: labels.voiceLabel, enabled: hasNovelText || hasVoiceLines }
  ]
