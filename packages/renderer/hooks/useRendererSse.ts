...
import { applyTaskLifecycleToOverlay } from '@/lib/query/task-target-overlay'
...
import { buildProjectSseUrl } from '@renderer/clients/sse-client'

export interface RendererSseOptions {
...
export function useRendererSse(options: RendererSseOptions) {
  const { projectId, episodeId, enabled = true, onEvent } = options
  ...
  const url = useMemo(() => {
    ...
    return buildProjectSseUrl(projectId, episodeId)
  }, [episodeId, projectId])
  ...
  useEffect(() => {
    ...
    const handleEvent = (event: MessageEvent) => {
      try {
        ...
        _ulogError('[useRendererSse] failed to parse event', error)
      }
    }
    ...
    source.onerror = (error) => {
      _ulogError('[useRendererSse] stream error', error)
    }
  ...
  }, [enabled, episodeId, isGlobalAssetProject, onEvent, projectId, queryClient, url])
