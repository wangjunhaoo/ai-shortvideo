...
import { cancelRun, retryRunStep } from '@renderer/clients/run-client'
...
export function useRunStreamState<TParams extends Record<string, unknown>>(
    ...
    async (params: TParams): Promise<RunResult> => {
      ...
      try {
        ...
        return await executeRunRequest({
          executeRequest: () => executeRequest(projectId, requestBody, controller.signal),
          controller,
          taskStreamTimeoutMs: TASK_STREAM_TIMEOUT_MS,
          applyAndCapture: applyEvent,
          finalResultRef,
  ...
  const retryStep = useCallback(async (params: {
    ...
    const response = await retryRunStep(runId, stepId, {
  ...
  const stop = useCallback(() => {
    ...
    if (runningRunId) {
      void cancelRun(runningRunId).catch(() => null)
      applyEvent({
        runId: runningRunId,
        event: 'run.error',
        ts: new Date().toISOString(),
