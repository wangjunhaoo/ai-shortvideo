...
import { fetchTaskTargetStates } from '@renderer/clients/task-client'
...
const taskTargetStateLogger = createScopedLogger({
  module: 'renderer.use-task-target-state-map',
})
...
async function fetchTargetStatesChunk(
  ...
  const response = await fetchTaskTargetStates(projectId, targets)
  if (!response.ok) {
...
async function flushTaskTargetStateBatch(projectId: string) {
  ...
  try {
    ...
    for (let chunkIdx = 0; chunkIdx < chunks.length; chunkIdx += 1) {
      ...
      for (let i = 0; i < chunkTargets.length && i < chunkStates.length; i += 1) {
...
export function useRendererTaskTargetStateMap(
  projectId: string | null | undefined,
  targets: TaskTargetStateQuery[],
  options: {
    enabled?: boolean
  ...
  const getState = useMemo(
    () => (targetType: string, targetId: string) => byKey.get(stateKey(targetType, targetId)) || null,
    [byKey],
  )
