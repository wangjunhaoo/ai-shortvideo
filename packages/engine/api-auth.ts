...
import { withPrismaRetry } from '@engine/prisma-retry'
import { extractModelKey } from '@engine/config-service'
import { getAuthSession, type AuthSession } from '@engine/services/auth-session-service'
...
export { getAuthSession, type AuthSession } from '@engine/services/auth-session-service'
...
export async function requireProjectAuth<T extends ProjectAuthIncludes = ProjectAuthIncludes>(
    projectId: string,
    options?: { include?: T }
): Promise<ProjectAuthContextWithIncludes<T> | Response> {
    // 1. 验证 Session
    const session = await getAuthSession()
    if (!session?.user?.id) {
        return unauthorized()
 ...
 * ```
 */
export async function requireUserAuth(): Promise<{ session: AuthSession } | Response> {
    const session = await getAuthSession()
    if (!session?.user?.id) {
        return unauthorized()
    }
...
export async function requireProjectAuthLight(
    projectId: string
): Promise<{ session: AuthSession; project: { id: string; userId: string; name: string; [key: string]: unknown } } | Response> {
    const session = await getAuthSession()
    if (!session?.user?.id) {
        return unauthorized()
    }
 ...
 * 检查是否是错误响应
 */
export function isErrorResponse(result: unknown): result is Response {
