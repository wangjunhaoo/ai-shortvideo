...
- 2026-03-28 11:45:00：修复 packaged 首页客户端异常，根因是 URL 编码的 App Router chunk 路径被静态资源分发器当成不存在文件，落入 locale redirect 后变成 `/zh/_next/...` 404。
  - 更新 `desktop/runtime/static-asset-dispatcher.cjs`，在 `resolveSafeFile()` 中先对传入的资源相对路径执行 `decodeURIComponent`，再做安全归一化与根目录逃逸校验；这样 `/_next/static/chunks/app/%5Blocale%5D/*.js` 会正确命中磁盘上的 `.next/static/chunks/app/[locale]/*.js`，不再错误落回 307 locale redirect。
  - 更新 `scripts/desktop/smoke-test.mjs`，在原有 `/zh` HTML 校验之后新增 `verifyCriticalClientChunks()`：从返回的 HTML 中抽取 `/_next/static/chunks/app/%5Blocale%5D/*.js`，逐个以 `redirect: 'manual'` 请求并断言返回 `200 + javascript content-type`，把“页面 HTML 正常但 hydration chunk 404”这种客户端崩溃回归纳入 packaged smoke。
  - 使用浏览器实际验证 `http://127.0.0.1:13000/zh`：修复前 Playwright 控制台报 `ChunkLoadError`，`/_next/static/chunks/app/%5Blocale%5D/{layout,page}-*.js` 先 307 再 404；修复后页面标题恢复为 `AI动漫制作工作台`，控制台错误清零，这两条 chunk 直接返回 200。
  - 更新后的 `npm run desktop:smoke:packaged` 已再次通过，当前 `win-unpacked` 目录版首页客户端异常已消失。
- 2026-03-28 22:15:00：修复桌面安装包批量视频任务在 SQLite 上 `prisma.task.create()` 超时的问题，并顺手封住同类批量语音提交风险。
  - 新增 `src/lib/task/submit-concurrency.ts`，集中判断桌面本地任务或 SQLite `DATABASE_URL` 场景，并把批量任务提交并发收敛到 1；默认服务端环境仍保留较高并发。
  - 更新 `packages/engine/services/novel-promotion-video-task-service.ts` 与 `packages/engine/services/novel-promotion-voice-service.ts`，把原先 `Promise.all(...submitTask)` 的批量提交改成 `mapWithConcurrency(..., resolveBatchTaskSubmitConcurrency(), ...)`，避免多 panel / 多 voiceLine 同时写 `task` 表把桌面 SQLite 锁死。
  - 更新 `desktop/runtime/managed-web-runtime.cjs`，桌面 SQLite 的 `DATABASE_URL` 现在统一追加 `connection_limit=1&socket_timeout=30`，降低 Prisma SQLite 连接池竞争并拉长单机等待时间，避免短超时误判成 `Invalid prisma.task.create() invocation`。
  - 已通过 `npx vitest run tests/unit/novel-promotion/video-task-service.test.ts tests/unit/task/submit-concurrency.test.ts` 与 `npm run desktop:build:web`；重打包后需优先回归批量视频与批量语音提交链路。
